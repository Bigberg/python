...
get_id("../config/table.txt", "../config/staff_id.json")
...
while True:
	...
	if sql_dict["query_action"] == "select":
		...
		while True:
			with open("../config/staff_id.json", 'r') as load_f:
				...
				if sql_dict["query_object"] not in field_all_names_list \
					...
					if sql_dict["query_object_matching"] == "=":
						for i in range(len(staff_info_list)):
							...
							# with open("../config/table.txt",'r',encoding="utf-8") as f:
							# 	lines = f.readlines()
							# with open("../config/table.txt",'w',encoding="utf-8") as w:
